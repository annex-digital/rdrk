...
// Swiftmailer
try {
  $config['swiftmailer.transport']['smtp_host'] = $platformsh->smtpHost;
}
...
// Elasticsearch
$relationship_name = 'search_service';
$es_cluster_name = 'xxx';

if ($platformsh->hasRelationship($relationship_name)) {

  $platformsh->registerFormatter('drupal-elastic', function($search_credentials) {
    return sprintf('http://%s:%s', $search_credentials['host'], $search_credentials['port']);
  });
...
// Raven
$config['raven.settings']['environment'] = getenv(PLATFORM_ENVIRONMENT);
$config['raven.settings']['mapping']['client_key'] = getenv(SENTRY_DSN);
